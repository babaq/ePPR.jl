...
function plotphi(model::ePPRModel,xrange=-200:200)
    ...
    for t in 1:length(model)
        j=js[t];i=is[t]
        plot!(p[j,i],x->model.phi[t](x),minx,maxx,seriestype=:line,linewidth=2,link=:all,framestyle=:axes,title="β=$(round(model.beta[t],digits=3))")
...
function plotcor(models::Vector{ePPRModel},cors)
    x = length.(models)
    scatter(x,cat(cors...,dims=2)',ylabel="Correlation Coefficients",xlabel="Number of Terms",leg=false,xtick=x)
end
